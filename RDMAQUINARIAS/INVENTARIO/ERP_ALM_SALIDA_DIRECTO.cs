...
{
    public partial class ERP_ALM_SALIDA_DIRECTO: Form
    {
        public ERP_ALM_SALIDA_DIRECTO()
        {
            InitializeComponent();
        }
        NEG_ALM_PRODUCTOS neg = new NEG_ALM_PRODUCTOS();
        NEG_ALM_ALMACENES negAlmacen = new NEG_ALM_ALMACENES();
        NEG_ALM_SALIDA_DIRECTO negAlm = new NEG_ALM_SALIDA_DIRECTO();
        ...
        string tiMov;
        int opc;
        private void ERP_ALM_SALIDA_DIRECTO_Load(object sender, EventArgs e)
        {
            #region LLENAR COMBO
            /*negOpe.Opcion = 0;
            negOpe.Criterio = "";
        ...
        private void listarGrid(int opcion, string criterio,string tiMov)
        {
            try
            {
                ...
                negAlm.CoSuc = "";
                DataTable dt = DAT_ALM_SALIDA_DIRECTO.SP_ERP_ALM_MOVIMIENTO_SALIDA_CAB_LS(negAlm);
        ...
        private void listarOperacionesFlowSalida()
        {
            try
            {
                Button btn;
                ...
                flowOperacionesSal.Controls.Clear();
                for (int i = 0; i < dtOpe.Rows.Count; i++)
                {
                    ...
                    btn.Width = 120;
                    btn.Height = 80;
                    btn.Click += new EventHandler(btnSeleccionarOperacionSalida_Click);
                    flowOperacionesSal.Controls.Add(btn);
                }
            }
            catch (Exception ex)
            {
        ...
        private void btnSeleccionarOperacionSalida_Click(object sender, EventArgs e)
        {
            Button btn = sender as Button;
            coOpe = btn.Name;
            tiMov = "S";
        ...
        private void vistaFormulario()
        {
            ...
            fun.habilitarBotonesVista(btnBuscar, txtBuscar, false);
        }
        ...
        private void listarCabeceraParaFormulario(string nuMov)
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                negAlm.CoEmp = CLASES.ERP_GLOBALES.CoEmp;
                DataTable dtDetPro = DAT_ALM_SALIDA_DIRECTO.SP_ERP_ALM_MOVIMIENTO_SALIDA_DET_LS(negAlm);
        ...
        private void grabar()
        {
            try
            {
                if (opc == 1)
                {
                    ...
                    negAlm.CoEmp = CLASES.ERP_GLOBALES.CoEmp;
                    int i = Convert.ToInt32(DAT_ALM_SALIDA_DIRECTO.SP_ERP_ALM_NUMERADOR_GENERAR_SALIDA(negAlm).Rows[0]["num"]);
                    txtnumero.Text = i.ToString("00000000");
                    txtnuMov.Text = coOpe + "-" + cboseNum.SelectedValue.ToString() + "-" + txtnumero.Text;
                }

                SqlCommand cmdc = new SqlCommand("SP_ERP_ALM_MOVIMIENTO_SALIDA_CAB_GB", cn);
                ...
                for (int i = 0; i < dgvDetProductos.Rows.Count - 1; i++)
                {
                    SqlCommand cmdd = new SqlCommand("SP_ERP_ALM_MOVIMIENTO_SALIDA_DET_LS", cn);
